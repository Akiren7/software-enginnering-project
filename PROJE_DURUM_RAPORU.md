...
```json
{
  ...
  "credential_sig": "<hmac_hex>",
  "auth_signature": "<hmap_hex>"
}
```
...
17 NİSAN 2026 NOTLARI:

İşte Claude Code modeline okutman için sistemin son teknolojik sıçramasını özetleyen 17 Nisan rapor eklentisi. Bu metni doğrudan kopyalayıp mevcut raporunun en altına yapıştırabilirsin:

🚀 17 NİSAN 2026 GÜNCELLEMELERİ (Session Manager & Kurumsal Entegrasyon)
Bu aşamada sistem basit bir "veri yönlendirici" (routing server) olmaktan çıkıp, tüm sınav yaşam döngüsünü kontrol eden Gelişmiş Oturum Yöneticisi (Session Manager) mimarisine geçiş yapmıştır.

1. Kurumsal Kimlik Doğrulama: Web Scraping ile CATS/Orion Entegrasyonu
   school_service.py Eklendi: Okulun resmi bir API'si olmadığı için requests ve BeautifulSoup4 kütüphaneleri kullanılarak Web Scraping (Veri Kazıma) mimarisi kuruldu.

Canlı Doğrulama: İstemciden gelen öğrenci numarası (eid) ve şifre (pw), arka planda doğrudan İKÜ CATS (Sakai LMS) sistemine gönderiliyor.

HTML Parsing (Mert'in Önerisi): Başarılı giriş sonrası dönen HTML sayfası parçalanarak <div class="Mrphs-userNav__submenuitem--fullname"> etiketinden öğrencinin gerçek adı çekiliyor ve sisteme kaydediliyor.

Güvenlik (Impersonation Protection): Başkasının numarasıyla girmeye çalışanlar, CATS'ten dönen orijinal numara ile uyuşmazlık yaşadığı an sistem tarafından engelleniyor.

2. Gelişmiş Oturum Yönetimi (Session Lifecycle - server_core.py)
   Bekleme Odası (Waiting Room) Mantığı: Başarıyla doğrulanan öğrenciler artık direkt sınava (in_progress) başlamıyor. Sunucu onları waiting_for_start durumuna alarak bekletiyor.

Küresel Koordinasyon (Broadcast): Eğitmenden gelen start_all_students komutu ile sunucu, o sınava kayıtlı tüm öğrencilere broadcast_to_exam fonksiyonu üzerinden exam_started_ack sinyali göndererek sınavı eşzamanlı başlatıyor.

Global Süre Yönetimi: Eğitmenler change_duration komutuyla aktif bir sınavın süresini tüm sınıf için anlık olarak uzatabiliyor.

Zamanlayıcı Optimizasyonu: global_timer içindeki sync_time sinyalleri sadece in_progress olan öğrencilere gönderilerek ağ verimliliği artırıldı.

3. Dirençli Ağ İletimi (Engin'in Fixleri & Sunucu Entegrasyonu)
   Sıra Numarası ve Oturum Takibi: Engin'in istemciye eklediği seq (sıra numarası) ve session_id (oturum kimliği) verileri artık sunucu tarafında yakalanıp active_students belleğine işleniyor.

Kuyruk (Buffering) Desteği: Bağlantı koptuğunda istemcide biriken paketler, sunucuya ulaştığında buffered=True bayrağıyla tanınıyor.

Zaman Tüneli Düzeltmesi: Gecikmeli gelen paketlerde, paketin sunucuya ulaştığı zaman değil, istemcide ilk oluşturulduğu zaman (queued_at) kullanılarak veritabanına (db_manager) doğru adli loglama yapılıyor.

4. İstemci (Client) ve Arayüz Güncellemeleri
   demo.py (Test Terminali): Arayüze CATS şifresi (password) alanı eklendi ve tüm doğrulama trafiği buna göre güncellendi. İstemcinin sınavın başlaması için sunucudan gelecek komutu beklemesi (while True: await ws.recv()) sağlandı.

Entegrasyon Gereksinimi (TODO): Engin'in network_sender.py (register_exam message) ve monitor_loop.py dosyaları, sunucunun yeni password beklentisine uyumlu hale getirilecek.
